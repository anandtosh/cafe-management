            ...
            <nav class="main-header navbar {{config('adminlte.classes_topnav_nav', 'navbar-expand-md')}} {{config('adminlte.classes_topnav', 'navbar-white navbar-light')}}">
                <ul class="navbar-nav">
                    ...
                    <li class="dropdown nav-item">
                        <button class="btn dropdown-toggle" type="button" data-toggle="dropdown">Fiscal Year
                            <span class="caret"></span></button>
                            <ul class="dropdown-menu">
                                @foreach (cache('configs')->where('type','FISCAL YEAR') as $item)
                                <a class="dropdown-item" type="button" href="{{url('admin/ses/fiscal/'.$item->name)}}">{{$item->name}} @if($item->name==session('fiscal'))<i class="fa fa-check" aria-hidden="true"></i>@endif</a>
                                @endforeach
                            </ul>
                    </li>
                    @hasanyrole('Admin|Developer')
                    <li class="dropdown nav-item">
                        <button class="btn dropdown-toggle" type="button" data-toggle="dropdown">Franchise
                            <span class="caret"></span></button>
                            <ul class="dropdown-menu">
                                @foreach (cache('franchises') as $item)
                                <a class="dropdown-item" type="button" href="{{url('admin/ses/franchise/'.$item->id)}}">{{$item->name}} @if($item->name==session('franchise'))<i class="fa fa-check" aria-hidden="true"></i>@endif</a>
                                @endforeach
                            </ul>
                    </li>
                    @endhasanyrole
                    ...
                    @if(Auth::user())
                        ...
                        <div class="dropdown">
                            <button class="btn btn-outline dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                             <i class="fa fa-user-secret" aria-hidden="true"></i>  Account
                            </button>

                            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                              {{-- <a class="dropdown-item" href="#">Action</a> --}}
                              <a class="dropdown-item" type="button"><i class="fa fa-user"></i> Profile</a>
                            <a class="dropdown-item" type="button" href="{{url('admin/changepw')}}"i class="fas fa-passport"></i> Change Password</a>
